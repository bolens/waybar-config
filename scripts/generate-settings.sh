...
build_system_json() {
  local app_open="${scripts}/app-open.sh"
  ...
  jq -n \
    ...
    def click_app($key): ($app_open + " " + app($key));
    {
      ...
      "custom/updates": {
        ...
        "on-click-middle": ($scripts + "/updates-status.sh --refresh")
      },
      "custom/ups": {
        format: "{}",
      ...
      "custom/uptime": {
        ...
        "on-click-middle": ($scripts + "/uptime-status.sh --refresh")
      },
      "custom/psu": {
        format: "{}",
        "return-type": "json",
        ...
        "on-click": click_app("system_monitor"),
        "on-click-right": click_app("btop"),
        "on-click-middle": ($scripts + "/psu-status.sh --refresh")
      },
      "custom/fans": {
        format: "{}",
        "return-type": "json",
        ...
        "on-click-right": click_app("btop"),
        "on-click-middle": ($scripts + "/fans-status.sh --refresh")
      },
...
if [ -x "$WAYBAR_HOME/scripts/generate-network-modules.sh" ]; then
  "$WAYBAR_HOME/scripts/generate-network-modules.sh"
fi

if [ -x "$WAYBAR_HOME/scripts/generate-dock-modules.sh" ]; then
  "$WAYBAR_HOME/scripts/generate-dock-modules.sh"
fi

if [ -x "$WAYBAR_HOME/scripts/generate-module-configs.sh" ]; then
  "$WAYBAR_HOME/scripts/generate-module-configs.sh"
fi
