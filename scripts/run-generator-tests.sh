...
WAYBAR_HOME="$TEST_DIR" bash -c ". '$TEST_DIR/scripts/waybar-settings.sh'; waybar_settings_get '.bars.layer' 'missing'" >/tmp/waybar-sot-layer.$$
...
WAYBAR_HOME="$TEST_DIR" bash -c ". '$TEST_DIR/scripts/waybar-settings.sh'" >/dev/null

# waybar_module_interval reads module_intervals (and maps once -> fallback)
ttl_weather=$(WAYBAR_HOME="$TEST_DIR" bash -c ". '$TEST_DIR/scripts/waybar-cache-helpers.sh'; waybar_module_interval weather 999")
ttl_once=$(WAYBAR_HOME="$TEST_DIR" bash -c ". '$TEST_DIR/scripts/waybar-cache-helpers.sh'; waybar_module_interval keyboard_layout 42")
ttl_missing=$(WAYBAR_HOME="$TEST_DIR" bash -c ". '$TEST_DIR/scripts/waybar-cache-helpers.sh'; waybar_module_interval totally_missing_key_xyz 77")
...
ttl_alias=$(WAYBAR_HOME="$TEST_DIR" bash -c ". '$TEST_DIR/scripts/waybar-settings.sh'; waybar_poll_interval weather 999")
...
if [ ! -x "$TEST_DIR/scripts/keyboard-layout-click.sh" ]; then
  echo "FAIL: keyboard-layout-click.sh missing or not executable" >&2
  fail=1
fi
if ! sh -n "$TEST_DIR/scripts/keyboard-layout-click.sh"; then
...
XDG_RUNTIME_DIR="$runtime_stub" "$TEST_DIR/scripts/listener-ctl.sh" start "$TEST_DIR/scripts/mock-listener.sh" mock-listener
...
if [ ! -f "$mock_pid_file" ] || ! kill -0 "$(cat "$mock_pid_file")" 2>/dev/null; then
  ...
  XDG_RUNTIME_DIR="$runtime_stub" "$TEST_DIR/scripts/listener-ctl.sh" stop mock-listener
  sleep 0.3
...
WAYBAR_MOCK_LOCK_NAME=device-notifier XDG_RUNTIME_DIR="$runtime_stub" \
  "$TEST_DIR/scripts/listener-ctl.sh" start "$TEST_DIR/scripts/mock-listener.sh" device-notifier
...
if [ ! -f "$dn_pid_file" ] || ! kill -0 "$(cat "$dn_pid_file")" 2>/dev/null; then
  ...
  XDG_RUNTIME_DIR="$runtime_stub" "$TEST_DIR/scripts/listener-ctl.sh" stop-all
  sleep 0.3
...
if ! grep -q 'def load_waybar_signals' "$TEST_DIR/scripts/active-window-listener-kde.py"; then
  echo "FAIL: active-window-listener-kde.py missing load_waybar_signals" >&2
  fail=1
fi
if ! grep -q 'waybar_rtmin("active_window")' "$TEST_DIR/scripts/active-window-listener-kde.py"; then
...
src = (test_dir / "scripts" / "active-window-listener-kde.py").read_text()
...
"$TEST_DIR/scripts/generate-settings.sh" >/dev/null
"$TEST_DIR/scripts/generate-compositor-modules.sh" >/dev/null
"$TEST_DIR/scripts/generate-workspaces-css.sh" >/dev/null

# validate-generated-config contract script (after full restore/regen)
if ! WAYBAR_HOME="$TEST_DIR" "$TEST_DIR/scripts/validate-generated-config.sh" >/dev/null; then
...
JSON
if ! "$TEST_DIR/scripts/generate-settings.sh" >/dev/null; then
  echo "FAIL: generate-settings.sh failed for polish defaults" >&2
  fail=1
fi
if ! "$TEST_DIR/scripts/generate-module-configs.sh" >/dev/null; then
...
"$TEST_DIR/scripts/generate-settings.sh" >/dev/null
"$TEST_DIR/scripts/generate-module-configs.sh" >/dev/null
"$TEST_DIR/scripts/generate-compositor-modules.sh" >/dev/null
clean_utils=$(python3 -c "import re; t=open('$TEST_DIR/modules/utilities.generated.jsonc').read(); t=re.sub(r'/\*.*?\*/', '', t, flags=re.S); t=re.sub(r'^\s*//.*$', '', t, flags=re.M); print(t)")
...
if ! "$ROOT_DIR/scripts/run-secrets-and-settings-tests.sh"; then
