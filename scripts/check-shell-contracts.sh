...
for listener in \
  "$ROOT/scripts/device-notifier-listener.sh" \
  "$ROOT/scripts/privacy-listener.sh" \
  "$ROOT/scripts/workspaces-hyprland-listener.sh"
do
...
if ! grep -q 'WAYBAR_LISTENER_LOCK_NAME' "$ROOT/scripts/dock-windows-listener-lock.sh"; then
...
if ! command -v dash >/dev/null 2>&1; then
  ...
  if ! XDG_RUNTIME_DIR="$runtime" WAYBAR_LISTENER_LOCK_NAME=contract-test \
    dash -c ". \"$ROOT/scripts/dock-windows-listener-lock.sh\"; test -f \"$lock_pid\"; kill -0 \"\$(cat \"$lock_pid\")\""; then
  ...
  if XDG_RUNTIME_DIR="$runtime" dash -c ". \"$ROOT/scripts/dock-windows-listener-lock.sh\" should-not-work" 2>/dev/null; then
  ...
  cp "$ROOT/scripts/listener-ctl.sh" "$ROOT/scripts/dock-windows-listener-lock.sh" "$scripts_stub/"
...
mkdir -p "$tmpdir/data" "$tmpdir/cache" "$tmpdir/scripts"
cp "$ROOT/scripts/i2pd-status.sh" \
  "$ROOT/scripts/updates-status.sh" \
  "$ROOT/scripts/github-status.sh" \
  "$ROOT/scripts/waybar-cache-helpers.sh" \
  "$ROOT/scripts/waybar-settings.sh" \
  "$ROOT/scripts/unicode-animations-lib.sh" \
  "$tmpdir/scripts/" 2>/dev/null || true
...
if [ ! -f "$tmpdir/data/waybar-settings.json" ] && [ -f "$tmpdir/data/waybar-settings.jsonc" ]; then
  WAYBAR_HOME="$tmpdir" bash "$tmpdir/scripts/waybar-settings.sh" >/dev/null 2>&1 || true
fi
...
cat >"$tmpdir/scripts/waybar-signal.sh" <<'EOF'
#!/usr/bin/env sh
exit 0
EOF
...
chmod +x "$tmpdir"/bin/* "$tmpdir/scripts/waybar-signal.sh"
...
neg="$tmpdir/scripts/i2pd-status-forced-sh.sh"
sed '1s|.*|#!/usr/bin/env sh|' "$tmpdir/scripts/i2pd-status.sh" >"$neg"
chmod +x "$neg"
neg_out="$(
  PATH="/usr/bin:/bin" HOME="$tmpdir" WAYBAR_HOME="$tmpdir" XDG_CACHE_HOME="$tmpdir/cache-neg" \
