...
if [ "${1:-}" != "--refresh" ]; then
  ...
  if [ "$active_state" = "active" ] || [ "$active_state" = "activating" ]; then
    ...
    last_scan_date="N/A"
    stamp_file="/var/lib/systemd/timers/stamp-libredefender-scan.timer"
    ...
    for i in $(seq 1 75); do # 75 * 0.2s = 15s
      ...
      jq -cn \
        --arg text "$spinner Libre" \
        --arg tooltip "LibreDefender Antivirus\nStatus: Scanning...\nLast Scan: $last_scan_date\n\nScan is running in background..." \
        --arg class "scanning" \
    ...
    "$script_dir/libredefender-status.sh" --refresh >/dev/null 2>&1 &
    exit 0
  fi
  ...
  jq -cn --arg text "󰒃 Libre" --arg tooltip "Checking libredefender..." --arg class "normal" \
    '{text:$text, tooltip:$tooltip, class:$class}'
  exit 0
fi
...
if [ "$active_state" = "active" ] || [ "$active_state" = "activating" ]; then
  ...
  class="warning"
  icon="󰒃"
elif [ "$elapsed" -gt 604800 ]; then # 7 days
  status_text="Scan Stale (> 7 days)"
  class="warning"
  icon="󰒃"
fi

text=$(printf '%s Libre' "$icon")
tooltip=$(printf 'LibreDefender Antivirus
Status: %s
Last Scan: %s (%s)
Result: %s
